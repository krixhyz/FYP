    ...
    @else
        ...
        <div class="space-y-3 mb-8">
            ...
            @foreach($cartItems as $item)
                ...
                @endphp
                <div class="bg-white shadow-[0_20px_40px_rgba(26,28,28,0.06)] p-5 flex flex-col md:flex-row md:items-center gap-4 transition-all duration-300" data-cart-item="{{$item->id}}">
                    ...
                    <form action="{{ route('cart.update', $item->id) }}" method="POST" class="flex items-center gap-2" data-cart-action="update">
                        ...
                        <div class="flex items-center gap-0">
                            <button type="button" onclick="const input = this.closest('form').querySelector('input[name=quantity]'); input.value = Math.max(1, parseInt(input.value) - 1)" class="w-8 h-8 bg-white border-2 border-gray-300 font-space font-bold text-lg flex items-center justify-center hover:border-[#006a38]">−</button>
                            <input type="number" name="quantity" value="{{ $qty }}" min="1" max="{{ $item->product->quantity }}" class="w-16 h-8 bg-[#f3f3f3] border-0 border-b-2 border-gray-400 text-center font-manrope text-sm focus:border-[#006a38] focus:outline-none">
                            <button type="button" onclick="const input = this.closest('form').querySelector('input[name=quantity]'); input.value = Math.min({{ $item->product->quantity }}, parseInt(input.value) + 1)" class="w-8 h-8 bg-white border-2 border-gray-300 font-space font-bold text-lg flex items-center justify-center hover:border-[#006a38]">+</button>
                        </div>
                    ...
                    <div class="text-right md:min-w-[120px]">
                        <p class="font-space font-bold text-lg text-[#006a38]">Rs. {{ number_format($lineTotal,2) }}</p>
                    </div>
        ...
        <div class="bg-[#f3f3f3] px-6 py-6 space-y-4">
            <div class="flex justify-between items-center">
                ...
                <p class="font-space font-bold text-2xl text-[#006a38]">Rs. {{ number_format($grandTotal,2) }}</p>
            </div>
