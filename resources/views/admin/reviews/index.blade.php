            ...
            <tbody class="divide-y divide-neutral-200">
                @forelse ($reviews as $review)
                    <tr class="hover:bg-neutral-50">
                        <td class="p-3 text-neutral-500">{{ $review->id }}</td>
                        <td class="p-3 font-medium">{{ $review->reviewer?->name ?? 'N/A' }}</td>
                        <td class="p-3">
                            <a href="{{ route('admin.users.show', $review->reviewee_id) }}"
                               class="text-[var(--reloop-green)] hover:underline">{{ $review->reviewee?->name ?? 'N/A' }}</a>
                        </td>
                        ...
                        <td class="p-3">
                            <div class="flex">
                                ...
                                @endfor
                                <span class="ml-1 text-neutral-600">{{ $review->rating }}/5</span>
                            </div>
                        </td>
                        <td class="p-3 text-neutral-700 max-w-xs truncate">{{ $review->body ?? '-' }}</td>
                        <td class="p-3 text-neutral-500 text-xs">{{ $review->created_at->format('Y-m-d') }}</td>
                    </tr>
                @empty
                    <tr><td colspan="7" class="p-5 text-center text-neutral-600">No reviews yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
